...
class WidgetSetupHelper {
  ...
  /// Opens the system pin sheet only if no widget is on the home screen yet.
  static Future<void> requestPinIfNeeded() async {
    await HomeWidgetService.syncOnAppLaunch();

    if (!Platform.isAndroid) return;
    if (await WidgetPinPref.hasWidgetOnHomeScreen()) return;
  ...
  static Future<void> addWidgetToHomeScreen(BuildContext context) async {
    ...
    final already = await WidgetPinPref.hasWidgetOnHomeScreen();
    if (already) {
      await HomeWidgetService.syncFromCache();
      if (context.mounted) {
        AppConstants.showToast('Widget already on home screen — content refreshed');
      }
      return;
    }
    ...
    if (context.mounted) {
      AppConstants.showToast('Tap Add on the system dialog to place the widget');
    }
  }
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child: Text('Got it', style: getMediumStyle(color: AppColors.primaryColor)),
          ),
        ],
      ),
    );
